...
function DefaultLoader() {
  return (
    <div className="bg-slate-950/80 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-xs">
      <div className="flex gap-1">
        <span className="bg-indigo-400/60 size-1.5 animate-pulse rounded-full" />
        <span className="bg-indigo-400/60 size-1.5 animate-pulse rounded-full [animation-delay:150ms]" />
        <span className="bg-indigo-400/60 size-1.5 animate-pulse rounded-full [animation-delay:300ms]" />
      </div>
    </div>
  );
}
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="focus-visible:ring-ring hover:bg-slate-800 text-slate-200 absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm transition-colors focus:outline-none focus-visible:ring-2"
    >
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-slate-950/95 text-slate-100 relative max-w-62 rounded-xl border border-white/10 p-3 shadow-xl backdrop-blur-md",
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-slate-900 border border-white/10 text-slate-100 pointer-events-none rounded-md px-2.5 py-1 text-xs shadow-lg",
        "animate-in fade-in-0 zoom-in-95 duration-200 ease-out",
        className,
      )}
    >
  ...
  return (
    <div
      className={cn(
        ...
        "text-slate-250 text-[10px] font-medium font-mono bg-slate-950/80 px-1.5 py-0.5 rounded border border-white/10",
...
function ControlGroup({ children }: { children: ReactNode }) {
  return (
    <div className="border-white/10 bg-slate-900 [&>button:not(:last-child)]:border-white/5 flex flex-col overflow-hidden rounded-md border shadow-sm [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  );
}
...
function ControlButton({
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "flex size-8 items-center justify-center transition-all text-slate-300",
        "first:rounded-t-md last:rounded-b-md",
        "hover:bg-slate-800 hover:text-white",
        "focus-visible:ring-indigo-500 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-inset",
  ...
  return (
    <ControlButton onClick={onClick} label="Reset bearing to north">
      ...
      >
        ...
        <path d="M12 22L16 12H12V22Z" className="fill-slate-500" />
        <path d="M12 22L8 12H12V22Z" className="fill-slate-600" />
      </svg>
    </ControlButton>
  );
}
...
function MapPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-slate-950/95 border border-white/10 text-slate-100 relative max-w-62 rounded-xl p-3 shadow-xl backdrop-blur-md",
        "animate-in fade-in-0 zoom-in-95 duration-200 ease-out",
        className,
      )}
    >
